...
- (OSStatus) playSound: (NSString*) file {
    ...
    AudioQueueRef queue;
    res = AudioQueueNewOutput(&dataFormat,
                              pullAudioCallback,
                              &player,
                              NULL,
                              NULL,
    ...
    res = sizesFromTime(player.audioFile, dataFormat, kBuffTime, &buffSize, &player.numPacketsToRead);
    if (res != noErr) {
        return res;
    }
...
OSStatus sizesFromTime(AudioFileID audioFile, AudioStreamBasicDescription desc,
