...
func newGame(in io.Reader) *game {
	...
	g.maze.addEdge(0, 6, 1)
	g.maze.addEdge(6, 3, 1)
	g.maze.addEdge(6, 2, 1)
	g.maze.addEdge(0, 7, 1)
	g.maze.addEdge(7, 5, 1)
	g.maze.addEdge(7, 1, 1)
...
func (g *game)placeBat() {
	for j := 0; j < numPlaceTries; j++ {
		r := g.randomRoom()
		if r != g.player && !g.hasBatIn(r) {
			g.bats[r] = struct{}{}
			return
		}
	}
	panic("could not place bat")
}

func (g *game)placePit() {
	for j := 0; j < numPlaceTries; j++ {
		r := g.randomRoom()
...
func (g *game)randomRoomNotIn(rooms map[int]struct{}) (int, error) {
	for i := 0; i < 10; i++ {
		r := rand.Intn(g.maze.len())
		_, ok := rooms[r]
		if !ok {
			return r, nil
		}
	}
	return 0, errors.New("could not place")
}
...
func (g *game)doRoomEnter() {
	if g.player == g.wumpus {
		...
		g.gameOver = true
	} else if g.hasBatIn(g.player) {
		fmt.Println("Aaaaahhhhhhhhh! Superbat attack!")
		delete(g.bats, g.player)
		g.player = g.randomRoom()
		g.placeBat()
		// Graph size should be big enough that this terminates.
		g.doRoomEnter()
	} else if g.hasPitIn(g.player) {
...
func (g *game)doRoomStatus() {
	...
	for _, e := range g.maze.edges(g.player) {
		...
		bat = bat || g.hasBatIn(e.end)
		pit = pit || g.hasPitIn(e.end)
	}

	if bat {
		fmt.Println("I hear bats!")
	}
	if pit {
...
func (g *game)hasBatIn(room int) bool {
	_, ok := g.bats[room]
	return ok
}
