...
//         [ "(" inside ")" ]
//
// inner := unary | binary | call | ifElseExpr | let
// unary := OP("-") expr
// binary := OP expr expr
// call := SYM expr*
// ifElseExpr := "if" expr expr expr
...
func parseInner(p *parser) (astExpr, error) {
	...
	if sym.symType == SYM_IF {
		return parseIfElseExpr(p)
	}
	if sym.symType == SYM_LIT {
		return nil, fmt.Errorf("a literal number is not allowed in this context. get rid of the parens?")
	}
	return nil, fmt.Errorf("expected a function, 'let', of 'if', but got %s", sym.value)
}
...
func parseIfElseExpr(p *parser) (astExpr, error) {
	// last sym read was SYM_IF and it is unconsumed
	// terminates successfully with final ')' still unconsumed
	p.consume() // eat 'if'
	sym := p.read()
	if sym.symType == SYM_CLOSE_PAREN {
		return nil, fmt.Errorf("if clause missing conditional, then, and else expressions")
	}
	...
	sym = p.read()
	if sym.symType == SYM_CLOSE_PAREN {
		return nil, fmt.Errorf("if clause missing then and else expressions")
	}
	ifExpr, err := parseExpr(p)
	if err != nil {
		return nil, err
	}
	...
	sym = p.read()
	if sym.symType == SYM_CLOSE_PAREN {
		return nil, fmt.Errorf("if clause missing else expressions")
	}
	elseExpr, err := parseExpr(p)
	if err != nil {
		return nil, err
	}

	return newIfElseExpr(cond, ifExpr, elseExpr), nil
